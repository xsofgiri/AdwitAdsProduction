...
ArrayList<NoteTeamLeadDesigner> qaNotesList = request.getAttribute(Constants.NOTES_TL_QA_LIST)!=null?(ArrayList<NoteTeamLeadDesigner>)request.getAttribute(Constants.NOTES_TL_QA_LIST):null;
ArrayList<NoteTeamLeadDesigner> designerNotesList = request.getAttribute(Constants.NOTES_TL_DESIGNER_LIST)!=null?(ArrayList<NoteTeamLeadDesigner>)request.getAttribute(Constants.NOTES_TL_DESIGNER_LIST):null;
...
<html lang="en">
	...
	<body id="kt_body" class="header-fixed header-tablet-and-mobile-fixed">
                                               ...
                                               <div class="row " >
                                                ...
                                                <div  class="row pt-2 ">
                                                 ...
                                                 <div class="col-md-4 col-xs-6 pt-2 pb-2 align-items-center text-center">
                                                            ...
                                                            <div class="col-md-4 col-xs-6 pt-2 pb-2 align-items-center text-center">
                                                                <button type="button" class=" btn btn-success btn-sm col-md-6 col-xs-10" data-bs-toggle="modal" data-bs-target="#kt_modal_send_to_qa">Send to QA</button>
                                                            </div>
                                                        </div>
                                              </div>
                                        </div>
      ...
      <div class="modal fade" tabindex="-1" id="kt_modal_back_to_designer">
          ...
          <div class="modal-body">
                    	<form name="qaOrderForm" id="qaOrderForm" action="<%=contextPath %>/designer/orders/designcheck/<%=EncryptionUtil.encode(order.getOrderId()) %>/" method="post">
                        ...
                        <div class="row pt-3 pb-3 ps-6 pe-6">
                               <!--begin::Form-->
                                ...
                                <div class="fv-row">
                                    <!--begin::Dropzone-->
                                    <div class="dropzone bg-white border-gray-400" id="kt_dropzonejs_tl_changes">
                                        <!--begin::Message-->
        ...
        <div class="modal fade" tabindex="-1" id="kt_modal_send_to_qa">
            <div class="modal-dialog modal-dialog-centered ">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Send to QA </h5>
                    ...
                    <div class="modal-body">
                    	<form name="qaOrderForm" id="qaOrderForm" action="<%=contextPath %>/designer/orders/designcheck/<%=EncryptionUtil.encode(order.getOrderId()) %>/" method="post">
                       		<input type="hidden" name="adStatus" id="adStatus" value="<%=Constants.SEND_TO_QA %>" />
                        
                        <div  class="row pt-2 pb-3 ">
                               
                            <div class="col-md-12 ps-6 pe-6">
                                <div class="d-flex flex-column" id="toastTypeGroup">
                                    <%if(qaNotesList!=null && qaNotesList.size()>0){
                                    	for(NoteTeamLeadDesigner noteTeamLeadDesigner : qaNotesList){
                                    	%>
                                    <label class="form-check form-check-custom form-check-solid form-check-sm  pt-3 pb-3">
                                        <input onchange="if($(this).is(':checked')) { $('#otherQANotes').hide(); }"  class="form-check-input" type="radio" value="<%=noteTeamLeadDesigner.getName() %>" name="adNotes" >
                                        <span class="form-check-label"><%=noteTeamLeadDesigner.getName() %></span>
                                    </label>
                                   		 <%}
                                    }%>
                                    ...
                                    <label class="form-check form-check-custom form-check-solid form-check-sm  pt-3 pb-3">
                                        <input onchange="if($(this).is(':checked')) { $('#otherQANotes').show(); }"  class="form-check-input" type="radio" value="Others" name="adNotes">
                        ...
                        <div class="row pt-3 pb-3 ps-6 pe-6" id="otherQANotes" style="display: none">
                            <label class="form-label">Ad Note</label>
                        <textarea class="form-control border-dashed" placeholder="Type here" id="adOtherNotes" name="adOtherNotes" style="height: 100px"></textarea>
                        </div>
                        
                        <div class="border-top ">
                        <div class="row pt-6 pb-6 align-items-center text-center">
                            <div class="col-md-12">
                                <button type="submit" class="btn btn-primary btn-sm col-md-5 col-xs-6">Send to QA</button>
                            </div>
        ...
        $("#kt_dropzonejs_tl_changes").dropzone({
        	url:"<%=contextPath %>/designer/orders/<%=EncryptionUtil.encode(order.getOrderId()) %>/uploadfile",
            addRemoveLinks: true,
            paramName: "tl_changes",
            maxFiles: 10,
            maxFilesize: 10, // MB
            success: function (file, response) {
                var imgName = response;
            ...
            error: function (file, response) {
	        	...
	        	console.log('remove file : '+name);
	        	 $.ajax({
	                    type: 'post',
	                    url:"<%=contextPath %>/designer/orders/<%=EncryptionUtil.encode(order.getOrderId()) %>/deletefile",
	                    data: {dFile:name, sPath:'TL_CHANGES'},
	                });
